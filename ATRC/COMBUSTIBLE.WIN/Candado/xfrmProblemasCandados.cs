...
namespace COMBUSTIBLE.WIN
{
    public partial class xfrmProblemasCandados : xfrmBase
    {
        ...
        private void ActualizarLista()
        {
            ...
            GO.Operands.Add(new BinaryOperator("Verificado", false));
            //GO.Operands.Add(new BinaryOperator("Unidad.Nombre", "C-154"));
            XPView Detalles = new XPView(Unidad, typeof(DetallesCandados), "Oid;FechaAlta;Unidad.Oid;Unidad.Nombre;Problema", GO);

            foreach (ViewRecord viewCandados in Detalles)
            {
                Candados Candado = new Candados();
                Candado.Oid = Convert.ToInt32(viewCandados["Oid"]);
                Candado.FechaAlta = Convert.ToDateTime(viewCandados["FechaAlta"]);
